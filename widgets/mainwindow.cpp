...
//----------------------------------------------------------------------
    ...
    if(is_73 and (m_config.disable_TX_on_73() or b)) {
      ...
      if(m_nextCall!="") {
        ...
        //auto_tx_mode (false);   //avt 10/31/21 deletion
        if(b) {
          m_ntx=6;
          ui->txrb6->setChecked(true);
          m_QSOProgress = CALLING;
...
//----------------------------------------------------------------------
  ...
  if (g_iptt == 1 && m_iptt0 == 0) {
    ...
    if(m_config.watchdog () && m_mode!="WSPR" && m_mode!="FST4W"
      ...
      m_txTimeout = 0;     //avt 11/2/21
      m_msgSent0 = current_message;
    }
    else
    {
      m_txTimeout++;    //avt 11/2/21
    }
    watchdog_label.setText (tr ("%1").arg(m_txTimeout));      //avt 11/2/21  tempOnly
    watchdog_label.setVisible (true);      //avt 11/2/21  tempOnly
...
void MainWindow::stopTx()
{
  ...
  if (m_auto && m_txTimeout >= maxTxTimeout)      //avt 6/5/22
  {
    ...
    m_QSOProgress = CALLING;    //avt 11/1/21
    m_bAutoReply = false;    //avt 11/2/21
    m_bCallingCQ = true;        //avt 11/2/21 allows tail-enders to be picked up
    ui->cbFirst->setStyleSheet ("QCheckBox{color:red}");    //avt 11/2/21
    watchdog_label.setText (tr ("T/O"));      //avt 11/2/21  tempOnly
    watchdog_label.setVisible (true);           //avt tempOnly
    m_txTimeout = 0;
    //QApplication::beep();     //avt tempOnly
  }
...
void MainWindow::stopTx2()
{
  ...
  //last_tx_label.setText(tr ("Last Tx: %1").arg (m_currentMessage.trimmed()));   //avt 11/1/21 deletion
}
...
void MainWindow::on_actionJT4_triggered()
{
  ...
  if(bVHF) {
    //                       01234567890123456789012345678901234567
    displayWidgets(nWidgets("11111001011011011011110000010000000000"));     //avt 11/3/21
  } else {
    //                       01234567890123456789012345678901234567
    displayWidgets(nWidgets("11101000010011000011000000010000000000"));     //avt 11/3/21
  }
  fast_config(false);
  statusChanged();
}
...
void MainWindow::on_actionJT9_triggered()
{
  ...
  if(m_bFast9) {
    ...
    //ui->cbAutoSeq->setChecked(false);   //avt 11/8/21 deletion
    if (m_mode != "FST4")
      {
        m_TRperiod=60.0;
        ui->lh_decodes_headings_label->setText("UTC   dB   DT Freq    " + tr ("Message"));
  ...
  if(bVHF) {
    //                       01234567890123456789012345678901234567
    displayWidgets(nWidgets("11111010110011111001000000010000000000"));     //avt 11/3/21
  } else {
    //                       01234567890123456789012345678901234567
    displayWidgets(nWidgets("11101000010011100001000000010000100000"));     //avt 11/3/21
  }
  fast_config(m_bFastMode);
  //ui->cbAutoSeq->setVisible(m_bFast9);     //avt 11/3/21 deletion
  statusChanged();
}
...
void MainWindow::on_actionJT65_triggered()
{
  ...
  if(bVHF) {
    //                       01234567890123456789012345678901234567
    displayWidgets(nWidgets("11111001010011011010110001010000000000"));     //avt 11/3/21
  } else {
    //                       01234567890123456789012345678901234567
    displayWidgets(nWidgets("11101000010011100001000000010000100000"));   //avt 11/3/21
  }
  ...
  if(ui->cbShMsgs->isChecked()) {
    //ui->cbAutoSeq->setChecked(false);   //avt 11/8/21 deletion
    //ui->cbAutoSeq->setVisible(false);   //avt 11/8/21 deletion
  }
  statusChanged();
}
...
// Make sure that MSK144 is not checked.
  ...
  //                       01234567890123456789012345678901234567
  displayWidgets(nWidgets("10111111010000000001000100011000000000"));   //avt 12/10/21
  fast_config(m_bFastMode);
  statusChanged();
  ...
  if (triggered)
    {
      ...
      if (m_tune) stop_tuning ();
      if (m_auto) auto_tx_mode (false);
      tx_status_label.setStyleSheet ("QLabel{color: #000000; background-color: #ff0000}");      //avt 11/1/21 deletion
...
void MainWindow::on_cbAutoSeq_toggled(bool b)
{
  ...
  ui->cbFirst->setVisible((m_mode=="FT8" or m_mode=="FT4" or m_mode=="FST4"
                           or m_mode=="Q65" or m_mode=="JT65" or m_mode=="JT9" or m_mode=="JT4" or m_mode=="MSK144") and b);      //avt 12/10/21
}
